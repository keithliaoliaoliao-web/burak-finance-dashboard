...
IGNORE_SYMBOLS = {
    ...
    "NEW", "HOLD", "BUY", "SELL", "CALL", "PUT", "TECH", "EV", "SAFE"
}
...
def extract_all_dynamic_tickers(tweets, sentiment_cache):
    ...
    print(f"🎯 全面掃描完成！共挖掘出 {len(sorted_tickers)} 個動態美股標的：{', '.join(sorted_tickers[:15])}...", flush=True)
...
def generate_html_dashboard(tweets, sentiment_cache, quotes):
    ...
    for sym, q in quotes.items():
        ...
        quotes_cards += f"""
        <div class="bg-slate-800/80 border border-slate-700/60 rounded-xl p-4 flex justify-between items-center shadow-lg hover:border-sky-500/50 transition">
            <div>
                <div class="text-xs font-semibold text-slate-400 tracking-wider">${sym}</div>
                <div class="text-xl font-bold text-white">${q['price']:.2f}</div>
            </div>
            <div class="text-right">
                <span class="inline-block px-2 py-0.5 rounded text-xs font-semibold {color_class} bg-slate-900/60 border border-slate-700/50">
                    {sign}{q['change_pct']:.2f}%
                </span>
                <div class="text-[10px] text-slate-500 mt-1">52W: {q['year_low']} - {q['year_high']}</div>
            </div>
        </div>
        """
    ...
    for tw in tweets[:60]:
        ...
        tweets_list_html += f"""
        <div class="bg-slate-800/60 border border-slate-700/60 rounded-xl p-5 hover:border-slate-600 transition shadow-md">
            <div class="flex justify-between items-start mb-3">
                <span class="px-2.5 py-0.5 rounded-full text-xs font-medium border {badge_color}">
                    情緒判定：{sentiment}
                </span>
                <span class="text-xs text-slate-500">{created_at.replace('T', ' ').replace('Z', '')}</span>
            </div>
            {f'<p class="text-sm font-semibold text-sky-300 mb-2">💡 AI 觀點：{summary}</p>' if summary else ''}
            ...
            <div class="mt-4 pt-3 border-t border-slate-700/40 flex justify-between items-center text-xs text-slate-400">
                ...
                <a href="{url}" target="_blank" class="text-sky-400 hover:text-sky-300 transition">查看原推文 ↗</a>
            </div>
        </div>
        """
...
<head>
    ...
    <title>Burak Finance 全球市場與推文情報儀表板</title>
...
<body class="bg-slate-900 text-slate-100 min-h-screen p-4 sm:p-8 font-sans">
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b border-slate-800 gap-4">
            ...
            <div class="text-xs text-slate-400 bg-slate-800/80 px-3 py-1.5 rounded-lg border border-slate-700/60">
